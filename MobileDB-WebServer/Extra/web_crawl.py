				...
				if re_obj:
					...
					for qual in err:
						if data_dict.has_key(qual):
							...
							if robj1:
								data_dict[qual] = unicode('Yes ,' + robj1.group(1),errors='replace')
							elif robj2:
								data_dict[qual] = unicode(robj2.group(1).split(',')[0],errors='replace')
					if data_dict.has_key("Price group"):
						...
						data_dict['Price group'] = unicode(price[0].split(',')[0],errors='replace')
					if data_dict.has_key('3.5mm jack'):
						jack = data_dict.pop('3.5mm jack')
						data_dict['jack'] = unicode(jack,errors='replace')
